...
testResult_t TutelIntraStageGetBuffSize(size_t *sendcount, size_t *recvcount, size_t count, int nranks, int rank) {
  size_t paramcount, sendInplaceOffset, recvInplaceOffset;
  TESTCHECK(TutelIntraStageGetCollByteCount(sendcount, recvcount, &paramcount, &sendInplaceOffset, &recvInplaceOffset, count, nranks, rank));
  return testSuccess;
}
